import { useNavigate, useParams } from "react-router-dom";
import {
  MdEdit, MdArrowBack, MdViewList, MdOpenInNew, MdOutlineDirectionsCar,
} from "react-icons/md";
import useGetModel from "hooks/models/useGetModel";
import Loading from "../../../components/loading/Loading";

const Field = ({ label, value }) => (
  <div className="space-y-1">
    <p className="text-xs text-slate-400">{label}</p>
    <div className="text-sm font-medium text-slate-900 break-words">{value ?? "—"}</div>
  </div>
);

const Section = ({ title, children }) => (
  <div>
    <p className="text-sm font-semibold text-slate-900 mb-4">{title}</p>
    {children}
  </div>
);

const Divider = () => <div className="border-t border-slate-100 my-6" />;

const ModelProfilePage = () => {
  const { uid } = useParams(); const id = uid;
  const navigate = useNavigate();

  const { model, loading, error } = useGetModel(id);

  if (loading) return <Loading text="Loading model..." />;
  if (error || !model) return (
    <div className="flex items-center justify-center py-20 text-sm text-red-500">{error ?? "Model not found."}</div>
  );

  const yearRange = model.year_end
    ? `${model.year_start} – ${model.year_end}`
    : `${model.year_start} – Present`;

  return (
    <div className="max-w-5xl mx-auto space-y-4">

      {/* ── Identity card ────────────────────────────────────────────── */}
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 sm:p-6">
        <div className="flex items-start gap-4">

          {/* Icon */}
          <div className="flex-shrink-0">
            <div className="w-16 h-16 rounded-2xl bg-brand-50 border border-brand-100 flex items-center justify-center">
              <MdViewList size={28} className="text-brand-500" />
            </div>
          </div>

          {/* Info */}
          <div className="flex-1 min-w-0">
            <h1 className="text-lg font-bold text-slate-900 leading-tight">{model.name}</h1>
            <p className="text-sm text-slate-400 mt-0.5">{yearRange}</p>
            <div className="flex flex-wrap items-center gap-1.5 mt-3">
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-semibold border bg-brand-50 text-brand-600 border-brand-200">
                {model.brand_name}
              </span>
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-semibold border bg-slate-50 text-slate-500 border-slate-200">
                {model.category}
              </span>
              {!model.year_end && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-semibold border bg-green-50 text-green-600 border-green-200">
                  <span className="w-1.5 h-1.5 rounded-full bg-green-500" />
                  In Production
                </span>
              )}
            </div>
          </div>

          {/* Actions */}
          <div className="flex flex-col gap-2 flex-shrink-0">
            <button type="button"
              onClick={() => navigate(`/admin/models/${id}/edit`)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-md lg:rounded-lg bg-brand-500 hover:bg-brand-600 text-xs font-medium text-white transition">
              <MdEdit size={13} /> Edit
            </button>
            <button type="button"
              onClick={() => navigate("/admin/models")}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-md lg:rounded-lg border border-slate-200 text-xs font-medium text-slate-600 hover:bg-slate-50 transition">
              <MdArrowBack size={13} /> Back
            </button>
          </div>
        </div>
      </div>

      {/* ── Details card ─────────────────────────────────────────────── */}
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 sm:p-6">

        <Section title="Model Details">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-8 gap-y-5">
            <Field label="Brand"        value={model.brand_name} />
            <Field label="Model Name"   value={model.name} />
            <Field label="Category"     value={model.category} />
            <Field label="Year Start"   value={model.year_start} />
            <Field label="Year End"     value={model.year_end ?? "Present (in production)"} />
          </div>
        </Section>

        <Divider />

        <Section title="Specifications">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-8 gap-y-5">
            <Field label="Transmission" value={model.transmission} />
            <Field label="Fuel Type"    value={model.fuel_type} />
            <Field label="Seats"        value={model.seats ? `${model.seats} seats` : null} />
          </div>
        </Section>

        <Divider />

        <Section title="Brand">
          <div className="flex items-center gap-3 rounded-xl bg-slate-50 border border-slate-100 px-4 py-3">
            <div className="w-8 h-8 rounded-lg bg-brand-50 border border-brand-100 flex items-center justify-center flex-shrink-0">
              <MdOutlineDirectionsCar size={16} className="text-brand-500" />
            </div>
            <div>
              <p className="text-sm font-semibold text-slate-900">{model.brand_name}</p>
              <p className="text-xs text-slate-400">Parent brand</p>
            </div>
            <button
              type="button"
              onClick={() => navigate(`/admin/brands/${model.brand_uid}`)}
              className="ml-auto flex items-center gap-1 text-xs text-brand-500 hover:text-brand-700 transition font-medium"
            >
              View brand <MdOpenInNew size={12} />
            </button>
          </div>
        </Section>

      </div>
    </div>
  );
};

export default ModelProfilePage;
