...
import {
  ...
  MdFilterList, MdClose, MdPeople, MdBusinessCenter, MdDirectionsCar, MdToggleOff,
} from "react-icons/md";
...
const roleMeta = {
  fleet_owner: { label: "Fleet Owner",       bg: "bg-brand-50",  text: "text-brand-600",  border: "border-brand-200"  },
  individual:  { label: "Individual Owner",  bg: "bg-slate-100", text: "text-slate-600",  border: "border-slate-200"  },
};
...
const PartnersPage = () => {
  ...
  const { partners, count, loading, error, params, setParams, refetch } = usePartners();
  const { deletePartner, loading: deleting } = useDeletePartner();

  const [deleteTarget, setDeleteTarget] = useState(null);
  const [filtersOpen, setFiltersOpen]   = useState(false);

  const fleetCount      = partners.filter((p) => p.role === "fleet_owner").length;
  const individualCount = partners.filter((p) => p.role === "individual").length;
  ...
  const handleDelete = async () => {
    ...
    const ok = await deletePartner(deleteTarget.uid);
    if (ok) {
      addToast(`${deleteTarget.name} has been deleted`, "success");
      setDeleteTarget(null);
      refetch();
    } else {
      addToast("Failed to delete partner. Please try again.", "error");
    }
  };
  ...
  return (
    <>
      ...
      <PageHeader
        title="Partners"
        subtitle="Manage fleet owners and individual vehicle owners"
        actions={
          <Button
            variant="primary"
            text="Add Partner"
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <StatCard icon={MdPeople}          label="Total Partners"    value={count}         />
        <StatCard icon={MdBusinessCenter}  label="Fleet Owners"      value={fleetCount}    accent />
        <StatCard icon={MdDirectionsCar}   label="Individual Owners" value={individualCount} />
      </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {!loading && (
            <span className="hidden sm:block text-xs text-slate-400 whitespace-nowrap flex-shrink-0">
              Showing{" "}
              <span className="font-semibold text-slate-600">{partners.length}</span>
              {" "}of{" "}
      ...
      <div className="bg-white rounded-2xl border border-slate-100">
        <div className="p-4 sm:p-6">
          <div className="rounded-xl border border-slate-100 overflow-hidden">
            ...
            {loading ? (
              <Loading text="Fetching partners data..." />
            ) : error ? (
              ...
              <EmptyState
                ...
                description="Try adjusting your search or filters to find partners."
              />
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-100 bg-slate-50/60">
                      {[
                        ...
                        { label: "Email",   icon: <MdDirectionsCar     size={13} /> },
                        { label: "Type",    icon: <MdBusinessCenter    size={13} /> },
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {partners.map((partner) => {
                      const meta = roleMeta[partner.role];
                      return (
                        <tr
                          key={partner.uid}
                        ...
                        >
                          ...
                          <td className="px-5 py-3.5">
                            <div className="flex items-center gap-3">
                              ...
                              <span className="font-semibold text-slate-900 truncate" title={partner.name}>
                                {partner.name}
                              </span>
                            </div>
                          </td>
                          ...
                          <td className="px-5 py-3.5 text-slate-500 max-w-[200px]">
                            <span className="truncate block" title={partner.email}>{partner.email}</span>
                          </td>
                          ...
                          <td className="px-5 py-3.5">
                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold border ${
                              ...
                              <span className={`w-1.5 h-1.5 rounded-full ${partner.is_active ? "bg-green-500" : "bg-slate-300"}`} />
                          ...
                          <td className="px-5 py-3.5" onClick={(e) => e.stopPropagation()}>
                            <div className="flex items-center gap-1">
                              <button
                                onClick={() => navigate(`/admin/partners/${partner.uid}/edit`)}
