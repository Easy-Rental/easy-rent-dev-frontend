...
const BrandEditPage = () => {
  ...
  const { brand, loading: loadingBrand, error: loadError } = useGetBrand(id);
  const { updateBrand, loading: updating, error: updateError, fieldErrors, getLastError } = useUpdateBrand();
  ...
  useEffect(() => {
    ...
    setLogoUrl(brand.logo?.public_url ?? "");
  }, [brand]);
  ...
  if (loadingBrand) return <Loading text="Loading brand..." />;
  if (loadError) return (
    ...
    <div className="max-w-5xl mx-auto">
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div className="px-4 sm:px-6 py-4 border-b border-slate-100">
          <h1 className="text-base font-bold text-slate-900">Edit Brand</h1>
          <p className="text-xs text-slate-400 mt-0.5">
            Updating <span className="font-semibold text-brand-600">{brand?.name}</span>
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-4 sm:px-6 py-6 space-y-6">
          ...
          <div className="flex gap-2 border-t border-slate-100 pt-5">
            ...
            <Button type="submit" variant="primary" text={updating ? "Saving..." : "Save Changes"} disabled={updating || !form.name.trim()} className="flex-1 py-2.5" />
          </div>
        </form>
      </div>
    </div>
