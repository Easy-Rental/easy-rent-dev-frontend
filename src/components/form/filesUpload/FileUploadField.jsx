mport { MdCloudUpload, MdDeleteOutline, MdInsertDriveFile, MdErrorOutline } from "react-icons/md";
        ...
        <div className={`rounded-xl border p-4 ${file.file_key ? "border-slate-200 bg-white" : "border-red-300 bg-red-50"}`}>
          <div className="flex items-center justify-between gap-3">
            <div className="flex min-w-0 items-center gap-3">
              <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${file.file_key ? "bg-green-100 text-green-600" : "bg-red-100 text-red-500"}`}>
          ...
          {file.uploading && (
            <div className={`mt-3 h-1 w-full rounded-full ${file.file_key ? "bg-green-100" : "bg-red-100"}`}>
              <div
                className={`h-full rounded-full ${file.file_key ? "bg-green-500" : "bg-red-500"} transition-all`}
