      ...
      {showExisting && (
        <div className="flex flex-col items-center gap-2">
          <div className="group relative">
            {imgError ? (
              <label className="flex h-24 w-24 cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-brand-400 bg-brand-50">
              ...
              <>
                <img src={existingUrl} alt="Current" className="h-24 w-24 rounded-xl object-cover ring-2 ring-brand-500 ring-offset-2" onError={() => setImgError(true)} />
      ...
      {showSimpleFile && (
        <div className="flex flex-col items-center gap-2">
          <div className="relative">
            <div className={`h-24 w-24 overflow-hidden rounded-xl shadow-md ring-2 ring-offset-2 ${simpleUploading ? "ring-brand-400" : "ring-green-400"}`}>
              {simpleFile.type?.startsWith("image/") ? (
                ...
                <div className={`flex h-full w-full items-center justify-center ${simpleUploading ? "bg-brand-50" : "bg-green-50"}`}>
                  <MdInsertDriveFile className={`h-6 w-6 ${simpleUploading ? "text-brand-500" : "text-green-500"}`} />
                </div>
              )}
            </div>
            {simpleUploading && (
        ...
        <div className={`rounded-xl border p-4 ${docFile.file_key ? "border-slate-200 bg-white" : "border-red-300 bg-red-50"}`}>
          <div className="flex items-center justify-between gap-3">
            <div className="flex min-w-0 items-center gap-3">
              <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${docFile.file_key ? "bg-green-100 text-green-600" : "bg-red-100 text-red-500"}`}>
          ...
          {docFile.uploading && (
            <div className={`mt-3 h-1 w-full rounded-full ${docFile.file_key ? "bg-green-100" : "bg-red-100"}`}>
              <div className={`h-full rounded-full ${docFile.file_key ? "bg-green-500" : "bg-red-500"} transition-all`} style={{ width: `${docFile.progress || 0}%` }} />
            </div>
          )}
        </div>
      )}
